...
export const propertyReferralsAPI = {
  ...
  export: async (params?: PropertyReferralQueryParams) => {
    const queryString = buildCleanQueryString(params);
    const url = `${import.meta.env.DEV ? '/api' : (import.meta.env.VITE_API_URL || 'https://api.jade-property.com/api/v1/admin')}/property-employee-referrals/export${queryString}`;
