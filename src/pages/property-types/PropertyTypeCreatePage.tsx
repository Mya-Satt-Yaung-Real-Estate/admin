...
import { useCreatePropertyType } from '../../services/queries/properties';
...
const PropertyTypeCreatePage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    createPropertyTypeMutation.mutate(
      ...
      },
      {
        ...
        onError: (error) => {
          const errorMessage = error?.message || 'Failed to create property type. Please try again.';
  ...
  return (
    <Box sx={{ marginLeft: 0, width: '100%' }}>
      <PageHeader
        ...
        breadcrumbs="Dashboard / Master Data / Property Types / Create"
        subtitle="Add a new property type"
        actionButton={{
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
              <Button
                ...
                onClick={() => navigate('/property-types')}
                disabled={createPropertyTypeMutation.isPending}
              >
                Cancel
              </Button>
              <Button
...
export default PropertyTypeCreatePage;
