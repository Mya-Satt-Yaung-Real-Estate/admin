...
import { useCreateWantingList } from '../../services/queries/wantingListings';
...
const WantingListingCreatePage: React.FC = () => {
  ...
  const handleSelectChange = (name: string, value: string | number) => {
    if (name === 'region_id') {
      setFormData({
        ...
        township_id: null, // Reset township when region changes
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.property_type_id === null || formData.property_type_id === undefined || formData.property_type_id === 0) {
    ...
    if (formData.min_budget === null || formData.min_budget <= 0) {
      alert('Min Budget is required and must be greater than 0');
      return;
    }

    if (formData.max_budget === null || formData.max_budget <= 0) {
      alert('Max Budget is required and must be greater than 0');
      return;
    }
    ...
    try {
      ...
      // Call the create mutation with the current form data
      await createWantingListMutation.mutateAsync(createData);

      // Navigate to the wanting lists list page with success message
      navigate(`/wanting-listings?success=${encodeURIComponent('Wanting list created successfully!')}`);
    } catch (error: any) {
      console.error('Error creating wanting list:', error);
  ...
  return (
    <Box>
      <PageHeader
        title="Create New Wanting List"
        breadcrumbs="Dashboard / Wanting List Management / Create Wanting List"
        actionButton={{
          text: 'Back to Wanting Lists',
          icon: <ArrowBackIcon />,
      ...
      {createWantingListMutation.isPending && (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 3 }}>
          <CircularProgress />
          <Typography variant="body1" sx={{ ml: 2 }}>Creating wanting list...</Typography>
        </Box>
      )}

      {/* Show error if there's an error */}
      {createWantingListMutation.isError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {createWantingListMutation.error?.message || 'An error occurred while creating the wanting list.'}
        </Alert>
      )}
          ...
          <Card>
            <CardContent>
              ...
              <Box sx={{ mt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                ...
                >
                  {createWantingListMutation.isPending ? 'Creating...' : 'Create Wanting List'}
                </Button>
              </Box>
            </CardContent>
          </Card>
...
export default WantingListingCreatePage;