...
import { useCreatePointPackage } from '../../services/queries/points';
import PageHeader from '../../components/layout/PageHeader';
import { PageLoadingState, ActionAlert } from '../../components/ui';
import { CreatePointPackageData } from '../../types/point';
import { useAlertSystem } from '../../hooks';

const PointPackageCreatePage: React.FC = () => {
  const navigate = useNavigate();
  
  // Alert system hook
  const { alert, showError, clearAlert } = useAlertSystem();
  
  const createPointPackageMutation = useCreatePointPackage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CreatePointPackageData = {
      ...
      await createPointPackageMutation.mutateAsync(submitData);
      
      // Navigate to point packages list on success
      navigate('/points/packages?success=' + encodeURIComponent('Point package created successfully!'));
    } catch (error: any) {
      ...
      if (error?.errors) {
        ...
        const errorMessage = error?.message || 'Failed to create point package. Please try again.';
  ...
  if (createPointPackageMutation.isPending) {
    return <PageLoadingState title="Creating Point Package" />;
  }
    ...
    <Box>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Error Alert */}
          {createPointPackageMutation.error && (
            <Alert severity="error" sx={{ mt: 3 }}>
              {createPointPackageMutation.error.message || 'Failed to create point package. Please try again.'}
            </Alert>
          )}

          {/* Success Alert */}
          {createPointPackageMutation.isSuccess && (
            <Alert severity="success" sx={{ mt: 3 }}>
              Point package created successfully!
            </Alert>
          )}
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', mt: 4 }}>
            <Button
              ...
              onClick={handleCancel}
              disabled={createPointPackageMutation.isPending}
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={createPointPackageMutation.isPending}
            >
              {createPointPackageMutation.isPending ? 'Creating...' : 'Create Point Package'}
            </Button>
          </Box>
        </form>
      </Paper>
...
export default PointPackageCreatePage;
